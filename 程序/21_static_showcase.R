...
.ghs_hero <- function(s, project_url, n_fig, n_widget) {
  ...
  sprintf(
    paste0(
      ...
      "<h1 class='hero-main' style='animation:ghs-hero-fade .9s ease .3s both'>\u5168\u7403\u536b\u751f\u652f\u51fa</h1>",
      "<p class='hero-sub' style='animation:ghs-hero-fade .9s ease .6s both'>Global Health Expenditure \u00b7 2000\u20132023</p>",
      "</div>",
      "</header>"
    ),
    dock_html
...
.ghs_css <- function() {
  paste(c(
    ...
    "a{color:var(--blue)}",
    ".hero{position:relative;min-height:100vh;padding:0;display:flex;align-items:center;justify-content:center;color:#1a1a1a;background:#f5f0e8;overflow:hidden}",
    ".hero:before{content:'';position:absolute;inset:0;background:radial-gradient(ellipse 70% 50% at 50% 45%,rgba(255,255,255,.5),transparent),radial-gradient(circle at 80% 85%,rgba(220,210,195,.4),transparent 50%);pointer-events:none}",
    ".hero:after{content:'';position:absolute;inset:0;background:url(\"data:image/svg+xml,%3Csvg width='80' height='80' xmlns='http://www.w3.org/2000/svg'%3E%3Ccircle cx='1' cy='1' r='.4' fill='rgba(0,0,0,.02)'/%3E%3C/svg%3E\");pointer-events:none}",
    ".hero-center{position:relative;z-index:2;text-align:center;max-width:900px;padding:0 24px}",
    ".hero-main{font-family:'Source Serif 4',Georgia,serif;font-size:clamp(56px,10vw,120px);font-weight:800;line-height:.95;letter-spacing:-.04em;color:#1a1a1a;margin:0}",
    ".hero-sub{font-family:'Inter',sans-serif;font-size:clamp(14px,1.8vw,20px);font-weight:400;letter-spacing:.12em;text-transform:uppercase;color:rgba(0,0,0,.4);margin:24px 0 0}",
