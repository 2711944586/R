...
.ghs_hero <- function(s, project_url, n_fig, n_widget) {
  ...
  sprintf(
    paste0(
      ...
      "<span class='hero-eyebrow' style='animation:ghs-hero-fade .8s ease .2s both'>GLOBAL HEALTH EXPENDITURE \u00b7 195 COUNTRIES \u00b7 23 YEARS \u00b7 INTEGRATED ANALYSIS</span>",
      "<h1 style='animation:ghs-hero-fade .8s ease .4s both'>",
      "<span class='hero-line1'>\u5168\u7403\u536b\u751f\u652f\u51fa</span>",
      "<span class='hero-line2'>2000\u20132023\uff1a<em>\u516c\u5e73\u3001\u97e7\u6027\u3001\u672a\u6765</em></span>",
      "</h1>",
      "</div>",
      "</header>"
    ),
...
.ghs_css <- function() {
  paste(c(
    ...
    "a{color:var(--blue)}",
    ".hero{position:relative;min-height:100vh;padding:0;display:flex;align-items:center;justify-content:center;background:#1c1917;overflow:hidden}",
    ".hero:before{content:'';position:absolute;inset:0;background:radial-gradient(ellipse 60% 50% at 50% 40%,rgba(60,50,40,.3),transparent),radial-gradient(circle at 75% 75%,rgba(40,35,30,.5),transparent 50%);pointer-events:none}",
    ".hero:after{content:'';position:absolute;inset:0;background:url(\"data:image/svg+xml,%3Csvg width='80' height='80' xmlns='http://www.w3.org/2000/svg'%3E%3Ccircle cx='1' cy='1' r='.3' fill='rgba(255,255,255,.015)'/%3E%3C/svg%3E\");pointer-events:none}",
    ".hero-center{position:relative;z-index:2;text-align:left;max-width:1100px;width:min(1100px,88vw);padding:0}",
    ".hero-eyebrow{display:inline-block;padding:8px 18px;border-radius:999px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);font-size:11px;letter-spacing:.14em;color:rgba(255,255,255,.45);font-weight:600;margin-bottom:36px}",
    ".hero h1{font-family:'Source Serif 4',Georgia,serif;font-size:clamp(48px,9vw,110px);font-weight:800;line-height:1.0;letter-spacing:-.03em;margin:0;color:#fafaf9}",
    ".hero .hero-line1{display:block}",
    ".hero .hero-line2{display:block;margin-top:.05em}",
    ".hero .hero-line2 em{font-style:normal;color:#c8956c}",
    ...
    "@keyframes ghs-hero-fade{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}}",
