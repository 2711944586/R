...
本轮调整把 Shiny 首页从“总览型信息页”改为纯标题封面：首页只保留项目名称与年份范围，通过字体层级和克制动画建立第一屏质感。静态 HTML 同步修复标题页动画、暖纸色背景、右下角 dock、图像放大弹层和交互组件空白问题；课程提交 HTML 内置轻量组件预览，离线打开时不会出现空白框，联网时仍可新窗打开完整 standalone widget。
...
提交包由 `程序/26_delivery.R` 生成，根目录只保留 `README.md` 一个文件；安装、启动、构建、依赖和索引文件已移动到 `项目入口/`、`交付索引/` 等文件夹中。当前提交包保留课程主报告、完整同源 Rmd、网站首页、Shiny 源码、核心派生数据、CSV 模型表、质量报告、项目文档和复现脚本，不再重复打包完整 widget 依赖、独立图片目录和重复代码副本。课程 HTML 和 Rmd 保留离线组件预览、dock 导航、图片放大和同一套暖色样式，二者保持同源同步。
