...
本轮调整把 Shiny 首页从“总览型信息页”改为纯标题封面：首页只保留项目名称与年份范围，通过字体层级和克制动画建立第一屏质感。分析内容、KPI、结论、路径卡片和入口说明均回到对应模块中，避免首页承担过多叙事功能。
...
提交包由 `程序/26_delivery.R` 生成，根目录只保留 `README.md` 一个文件；安装、启动、构建、依赖和索引文件已移动到 `项目入口/`、`交付索引/` 等文件夹中。当前提交包采用轻量策略：保留课程主报告、轻量 Rmd、网站首页、Shiny 应用、核心派生数据、CSV 模型表、质量报告、项目文档和复现脚本，不再重复打包完整 widget 依赖和完整 SVG 图表目录。
