...
本轮调整把 Shiny 首页从“总览型信息页”改为纯标题封面：首页只保留项目名称与年份范围，通过字体层级和克制动画建立第一屏质感。交互组件页新增精选组件工作台，动态气泡、地图、排行动画、旭日结构、平行坐标、相似网络、专题看板、热力图、雷达画像、树状结构、功能流图、液体仪表和主表浏览均可直接切换。
...
提交包由 `程序/26_delivery.R` 生成，根目录只保留 `README.md` 一个文件；安装、启动、构建、依赖和索引文件已移动到 `项目入口/`、`交付索引/` 等文件夹中。当前提交包保留课程主报告、完整同源 Rmd、网站首页、Shiny 源码、核心派生数据、CSV 模型表、质量报告、项目文档和复现脚本，不再重复打包完整 widget 依赖、独立图片目录和重复代码副本。课程 Rmd 由课程 HTML 抽取样式、正文和脚本生成，二者保持同一套内容和导航。
