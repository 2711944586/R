...
提交包由 `程序/26_delivery.R` 生成，根目录只保留 `README.md` 一个文件；安装、启动、构建、依赖和索引文件已移动到 `项目入口/`、`交付索引/` 等文件夹中。当前提交包保留课程主报告、完整同源 Rmd、网站首页、Shiny 应用、核心派生数据、CSV 模型表、质量报告、项目文档和复现脚本，不再重复打包完整 widget 依赖和完整图表目录。课程 Rmd 由课程 HTML 抽取样式、正文和脚本生成，二者保持同一套内容和导航。
